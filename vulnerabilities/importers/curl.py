...
from vulnerabilities.importer import (AdvisoryData, AffectedPackage, Importer,
                                      Reference, VulnerabilitySeverity)
from vulnerabilities.severity_systems import SCORING_SYSTEMS
from vulnerabilities.utils import fetch_response, get_item
...
def parse_advisory_data(raw_data) -> AdvisoryData:
    ...
    date_published = datetime.strptime(raw_data.get("published") or "", "%Y-%m-%dT%H:%M:%S.%fZ").replace(
        tzinfo=timezone.utc
    )
